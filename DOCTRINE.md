...
**Experience First.** Defaults must work for a first-time Ubuntu user after Apply. Chat on localhost. TTS that speaks. STT that transcribes. Do not leave a checkbox that only prints "binary not on PATH". Ship fewer polished workflows rather than ten broken frontends. Large vendor stacks such as OpenMOSS stay opt-in because of download size. When they are checked, they install fully.
